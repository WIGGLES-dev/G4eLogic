import { Data, OrArray, reduceToPathRecord, Crud, CrudEvent, CrudEventType, Resource } from "@internal"
...
interface Options { }
const connections: Set<Connection<any>> = new Set();
...
async function applySplitEntityMethods(methods: Record<string, Function>, data: OrArray<Data>, assignIds: boolean = false, map: (d: any) => any, options) {
    ...
    for (const parentId in embedded) {
        ...
        if (assignIds) assignAlternativeIds(embeddedItems);
    }
    ...
    if (assignIds) assignAlternativeIds([...newActors, ...newItems]);
}
...
function createCrudConnection() {
    ...
    const connection = connectToChild<any>({
        iframe,
