<script lang="ts">
    ...
    function hideAllUnfocusedNodes() {
        let nodes = draw.find(`
            [data-location-group]:not([data-location-group="${
    ...
    async function focusLocation(e) {
        ...
        let newLocation = $hitLocations$[
            locationGroup.dataset.locationGroup.split("-").join(" ")
        ];
        if (
            focusedLocation &&
            focusedLocation.name === newLocation.name
        ) return;
    ...
    $: getLocations = () => {
        const allSilhoueteLocations = [...leftLocations, ...rightLocations].reduce(
...
<style>
    ...
    :global(svg [data-location-group].crippled),
    :global(svg [data-location].crippled) {
        fill: red;
        stroke: #4a5568;
    }
