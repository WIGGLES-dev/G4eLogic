<script lang="ts" context="module">
    export function center(node: Element) {
        const { cx, cy } = node.root().viewbox();
        node.cx(cx).cy(cy);
    }
    function centerSVGElement(element: SVGElement) {
        const root = element.closest("svg");
        const { height, width, x, y } = root.viewBox.baseVal;
        const cx = x + width;
        const cy = y - height;
        element;
    }
    export async function frameInViewbox(node: Element, { scale = 1 } = {}) {
        ...
        return new Promise((resolve, reject) => {
            node.root()
                .animate(300)
                .viewbox(xStart, yStart, width, height)
                .after(resolve);
        });
    }
</script>
...
<script lang="ts">
    ...
    export let viewBox;
    export let scale;
    export let silhouette: SVGElement;
    export let draw;
    export let focusedLocation: string = null;
    $: style = `
        min-width:${minWidth};
    ...
    async function focusLocation(e) {
        ...
        let newLocation = locationGroup.dataset.locationGroup;
        if (focusedLocation === newLocation) {
            return;
        }
        focusedLocation = newLocation;
        const node = draw.find(`[data-location-group="${focusedLocation}"]`)[0];
        if (!node) {
            return;
        }
    ...
    function unfocusAll() {
        silhouette.querySelectorAll(`[data-location-group]`).forEach((node) => {
