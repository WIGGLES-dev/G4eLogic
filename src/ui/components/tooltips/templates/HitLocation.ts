...
export function hitLocationTemplate(location: HitLocation) {
    ...
    return `
        ...
        `<br/>
        <strong> DR ${location.armorValue()}</strong> - ${"PLACEHOLDER"} from natural DR.</br>
        <strong>${location.damageTaken}</strong> out of ${Math.floor(location.crippleThreshold())} to cripple<br/>
