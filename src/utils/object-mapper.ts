...
function ObjectMapper(src: object, dest: object): object;
function ObjectMapper<T>(src: object, dest: T, map: object): T
function ObjectMapper(src: object, dest: object, map?: object) {
...
function setKeyValue(dest, keystr, data, context = {} as any) {
...
function update_arr(dest, key, data, keys, context) {
    ...
    if (key.add) {
        if (data !== null && typeof data !== 'undefined') {
            ...
            dest = dest.concat(data.map(v => applyTransform(v, dest, context)))
        }
        return dest
    }
