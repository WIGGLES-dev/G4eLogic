...
export function getValueAtPath(object: object, path: Readonly<Path>) {
    return path.reduce(getValue, object);
}
export function getValuesFromPath(object: object, path: Readonly<Path>) {
    const values = [object];
    path.forEach((key, i) => {
...
export function deleteValueAtPath(object: object, path: Readonly<Path>) {
...
export function updateValueAtPath(object: object, path: Readonly<Path>, update: any | ((value: any) => any)) {
    const [key] = path.slice(-1);
    let value = object;
